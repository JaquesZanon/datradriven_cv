...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$entries_data <- read.csv("entries.csv", skip = 1,sep=';')
    cv$skills       <- read.csv("language_skills.csv", skip = 1,sep=';')
    cv$text_blocks  <- read.csv("text_blocks.csv", skip = 1,sep=';')
    cv$contact_info <- read.csv("contact_info.csv", skip = 1,sep=';')
  }
